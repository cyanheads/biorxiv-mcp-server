...
| `biorxiv_get_fulltext` | `rate_limited` | `RateLimited` | The full-text origin returned HTTP 429 for this host | Yes — wait the `retryAfter` seconds; `biorxiv_get_preprint` serves metadata meanwhile |
...
- **Rate limits undocumented, and the full-text origin enforces one** — neither `api.biorxiv.org` nor the `www.{server}.org` article pages publish a quota. The article pages sit behind a Cloudflare edge that returns HTTP 429 with a `Retry-After` after a handful of consecutive fetches. `biorxiv_get_fulltext` classifies that as a retryable `rate_limited` error carrying the wait, and its per-version extraction cache removes the repeat fetches that provoked it. `withRetry` honors `Retry-After` and fails fast when the requested wait exceeds its 30s cap, so no local backoff layer belongs here.
