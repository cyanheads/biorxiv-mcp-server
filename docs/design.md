...
| `biorxiv_get_preprint` | `invalid_doi_format` | `ValidationError` | Input DOI does not match `10.\d{4}/` pattern | No — fix and retry |
...
| `biorxiv_list_recent` | `invalid_category` | `ValidationError` | Category string not in taxonomy (server returns empty with no error) | No — use `biorxiv_list_categories` to get valid values |
